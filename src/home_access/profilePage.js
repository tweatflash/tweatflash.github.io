   ...
   const fetchPosts=async ()=>{
      try {
        const request =await axios.post(`/posts/user/${userAuth.user.username}`,{
          ...
          skipCount:profilePost ? profilePost.length :gwghwhgwh
        })
    ...
    useEffect(()=>{
      if (!pLoader && newPosts){
        dispatch(
          ProfilePostAdded([...userPosts ,...newPosts])
        )
      }
    },[pLoader])
          ...
          <div className='profile_wrapper'>
            <div className='cover_image'>
              <div className='cvr-img'>
                {userAuth.user.coverImage ? <AsyncImg srcImg={userAuth.user.coverImage}/> :<></>
                }
                <div className='profile_image'>
                  <div className='prf-img' onClick={()=>{
                    ...
                    {userAuth.user.profileImage ?  <AsyncImg srcImg={userAuth.user.profileImage}/>: <img style={ width <= 550 ? {width:"90px", height:"90px"}: {width:"100%", height:"100%"} } src={profileImg}/>}
                  </div>
            ...
            <div className={`profile-details-data ${ width <= 550 ? "profile-mbl": "profile-nml" }`}>
              <div className='profile-patch'>
                <button onClick={()=>navigator('edit')}>Edit Profile</button>
              </div>
              <div className='profile-details'>
                ...
                <div className='user_engaugement'>
                  <div className='user-eng' onClick={()=>navigator("following")}>{userAuth.user.following.length} &nbsp;
                    <p className='userName'>
                      <span>Following</span>
                    </p>

                  </div>
                  <div className='user-eng' onClick={()=>navigator("following")}>{userAuth.user.followers.length} &nbsp;
                    <p className='userName'>
                      <span>Followers</span>
                    </p>
                  </div>
                  <div className='user-eng' onClick={()=>navigator("following")}>{userAuth.user.friends.length} &nbsp;
                  <p className='userName'>
                      <span>Friends</span>
                    </p>
                  </div>
                    ...
                    <div className='posts-wrapper'>
                      <div className='post-list'>
                        <div className='posts-order-wrapper'>
                          {
                            profilePost.map((item,index)=>(<Feed item={item} index={index} type={"p"}/>))
                          }
                          <>
                            { !hro ?
