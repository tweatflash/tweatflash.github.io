...
const SinglePosts = () => {
    ...
    const uploadPostToBackend= async (postId)=>{
            ...
            try {
                const request =await axios.post(`/posts/commentOrReply/${postId}`,formData,{ headers: { 'Content-Type': 'multipart/form-data' } })
                const response=await request
                console.log(response)
                
                if (request.status===200){
                    setCommentData([{...response.data, replies:[]},...commentData])
                                                            ...
                                                            <div className='cmtr-upld-opt'>
                                                                {
                                                                    ...
                                                                    <input  id='img-upload2' type='file' ref={fRef} onChange={handleFileChange} className='file-selector' accept='image/jpeg,image/png,image/webp,image/gif' tabIndex={'-1'} />
